...
A) 官方 CosyVoice-1（FunAudioLLM/CosyVoice-300M，仓库默认 50Hz / vocab 4096）
   - flow 权重：与官方目录中的 flow.pt 一致（本机默认
     <CosyVoice-main>/pretrained_models/CosyVoice-300M/flow.pt）
...
【官方权重下载示例】（需在 CosyVoice-main 下存在 pretrained_models/）
  HF_HUB_ENABLE_HF_TRANSFER=0 huggingface-cli download FunAudioLLM/CosyVoice-300M \\
    --local-dir pretrained_models/CosyVoice-300M
...
示例（你的 Flow + S3 tokenizer，默认自动选 torch_ddp 下最新 epoch）：
  export PYTHONPATH=/mnt/data/ft_cosy:third_party/Matcha-TTS:$(pwd)
...
def _official_cosyvoice1_dir(repo: Path) -> Path:
    return repo / "pretrained_models" / "CosyVoice-300M"


# 自训 S3 tokenizer 默认探测路径（可用环境变量 COSYVOICE_S3_TOKENIZER_PT 覆盖）
_DEFAULT_S3_TOKENIZER_PT = Path("/mnt/data/ft_cosy/exp/step2_c_commit005/s3tokenizer_epoch10.pt")
...
def _ensure_import_paths():
    ...
    if matcha.is_dir() and any(matcha.iterdir()):
        ms = str(matcha)
        if ms not in sys.path:
            sys.path.insert(0, ms)
    # s3tokenizer_train lives next to CosyVoice-main (e.g. /mnt/data/ft_cosy/s3tokenizer_train).
    # Override: COSYVOICE_S3TOKENIZER_ROOT=/path/containing/s3tokenizer_train
    _s3_roots = []
...
def main():
    ...
    repo = _repo_root()
    # 默认走「你的」1024/25Hz + S3：yaml 与训练 config 一致；ckpt 默认取该实验 torch_ddp 下最新 epoch。
    ...
    if args.preset == "official_cosyvoice1_50hz":
        ...
        if args.tokenizer_pt.strip():
            ...
            elif _DEFAULT_S3_TOKENIZER_PT.is_file():
                args.tokenizer_pt = str(_DEFAULT_S3_TOKENIZER_PT)
            else:
                raise SystemExit(
