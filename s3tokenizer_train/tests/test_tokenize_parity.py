...
def _build_model(audio_ctx: int = 64) -> WhisperWithVQ:
    cfg = S3Config(
        n_mels=4,
...
def test_streaming_tokenize_matches_causal_offline_tokenize():
    model = _build_model(audio_ctx=64)
    mel = torch.randn(1, 4, 21)
    offline = model.tokenize(mel)
    state = model.init_stream_state(batch_size=1, device=mel.device, dtype=mel.dtype)
    chunks = [mel[:, :, :5], mel[:, :, 5:12], mel[:, :, 12:17], mel[:, :, 17:]]
    out = []
    for chunk in chunks:
...
def test_causal_prefix_invariance_no_future_leak():
    model = _build_model(audio_ctx=64)
    mel_a = torch.randn(1, 4, 16)
    mel_b = mel_a.clone()
    mel_b[:, :, 8:] = torch.randn_like(mel_b[:, :, 8:])

    tok_a = model.tokenize(mel_a)
    tok_b = model.tokenize(mel_b)

    # mel prefix length 8 -> stage1 length 4 -> tokenizer length 2
    assert torch.equal(tok_a[:, :2], tok_b[:, :2])


def test_legacy_tokenize_api_still_works_with_causal_path():
    model = _build_model(audio_ctx=64)
    mel = torch.randn(2, 4, 13)
    tokens = model.tokenize(mel)
    assert tokens.ndim == 2
    assert tokens.shape[0] == mel.shape[0]
