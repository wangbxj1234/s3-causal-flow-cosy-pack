...
def _sanitize_lengths(lengths: torch.Tensor, max_valid_len: int) -> torch.Tensor:
    # Guard against malformed dtype/content from data pipeline, which can
    # otherwise explode masks/interpolation (e.g. 1.0 bit-pattern as int32).
    return torch.clamp(lengths.to(torch.long), min=0, max=max_valid_len)


def _sanitize_feat_lengths(feat: torch.Tensor, feat_len: torch.Tensor) -> torch.Tensor:
    return _sanitize_lengths(feat_len, feat.shape[1])
...
class MaskedDiffWithXvec(torch.nn.Module):
    ...
    def forward(
        ...
        token_len = _sanitize_lengths(token_len, token.shape[1])
        feat_len = _sanitize_feat_lengths(feat, feat_len)
        ...
        # Always build mask with token's actual padded width. Some batches can
        # carry slightly inconsistent *_len metadata and otherwise cause
        # token/mask time-dim mismatch (e.g. 94 vs 93).
        ...
        # Defensive shape alignment: keep target and condition on identical time dim.
        if h.shape[1] != feat.shape[1]:
            target_len = min(h.shape[1], feat.shape[1])
            h = h[:, :target_len]
            feat = feat[:, :target_len]
            feat_len = torch.clamp(feat_len, max=target_len)
        ...
        mask = (~make_pad_mask(feat_len, max_len=feat.shape[1])).to(h)
...
class CausalMaskedDiffWithXvec(torch.nn.Module):
    ...
    def forward(
        ...
        h_out_len = _sanitize_lengths(h_lengths.sum(dim=-1).squeeze(dim=1), h.shape[1])
        mask = (~make_pad_mask(h_out_len, max_len=h.shape[1])).to(h)
        loss, _ = self.decoder.compute_loss(
            feat.transpose(1, 2).contiguous(),
            mask.unsqueeze(1),
            h.transpose(1, 2).contiguous(),
