...
class DistributedSampler:
    ...
    def sample(self, data):
        ...
        if self.partition:
            ...
            # Make shard count divisible by world_size so every rank consumes
            # the same number of source items and epoch-end synchronization
            # does not stall on uneven iterator exhaustion.
            remainder = len(data) % self.world_size
            if remainder != 0:
                pad = self.world_size - remainder
                data.extend(data[:pad])
