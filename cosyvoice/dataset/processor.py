...
def filter(data,
    ...
    """ Filter sample according to feature and label length
        ...
        Args::
            ...
            speech_token_max_length: drop utterance which is greater than
                speech_token_max_length for offline speech token training
    ...
    for sample in data:
        ...
        if online_feature is False and len(sample['speech_token']) == 0:
            continue
        if online_feature is False and len(sample['speech_token']) > speech_token_max_length:
            continue
...
def padding(data, use_spk_embedding, speech_feat_max_length=-1, speech_token_max_length=-1, mode='train', gan=False, dpo=False):
    ...
    for sample in data:
        ...
        if speech_feat_max_length is not None and speech_feat_max_length > 0:
            speech_feat = [i[:speech_feat_max_length] for i in speech_feat]
        ...
        if torch.tensor(['speech_token' in sample[i] for i in order]).all():
            ...
            if speech_token_max_length is not None and speech_token_max_length > 0:
                speech_token = [i[:speech_token_max_length] for i in speech_token]
